...
//   6. 位置和朝向均达标时才标记 finished=true；
//   7. 超过 goal_align_timeout_sec 时零速并标记 timed_out=true，
//      由上层进入 FAILED，不得将超时当作成功；
    ...
    double now_sec)
{
  ...
  result.position_reached = position_reached;
  result.yaw_reached = yaw_reached;
