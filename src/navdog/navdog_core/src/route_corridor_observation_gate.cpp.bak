    ...
    const RouteCorridorAssessment& observation) const noexcept
{
  ...
  if (!std::isfinite(observation.first_blocked_distance_ahead_m))
  {
    return false;
  }

  if (!std::isfinite(observation.first_blocked_arc_length_m))
  {
    return false;
  }

  // blocked == true requires finite blocked distances.
  if (observation.blocked)
  {
    if (std::isinf(observation.first_blocked_distance_ahead_m) ||
        std::isinf(observation.first_blocked_arc_length_m))
    {
      return false;
    }
  }

  // blocked == false requires infinite blocked distances.
  if (!observation.blocked)
  {
    if (!std::isinf(observation.first_blocked_distance_ahead_m) ||
        !std::isinf(observation.first_blocked_arc_length_m))
