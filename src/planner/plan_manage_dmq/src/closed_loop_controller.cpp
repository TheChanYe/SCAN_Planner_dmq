...
bool loadParams(const ros::NodeHandle &nh)
{
  ...
  nh.param("heading_resume_threshold", heading_resume_threshold, 0.35);
...
void cmdCallback(const ros::TimerEvent &)
{
  ...
  const double vyaw_cmd =
      clamp(kp_yaw * yaw_err, -max_vyaw, max_vyaw);
  ...
  /*
   * 航向误差越大，平移速度越低。
   * 不再使用“阈值以内全速、阈值以外全停”的突变控制。
   */
  const double heading_ratio = std::min(
      1.0,
      std::max(
          0.0,
          abs_yaw_err /
              std::max(heading_error_threshold, 1e-6)));
  ...
  const double heading_speed_scale =
      1.0 -
      (1.0 - min_heading_speed_scale) *
          smooth_ratio;

  /*
   * 同时减慢参考轨迹时间推进，防止机器人正在转弯时，
   * B-spline 参考点继续向前跑远。
   */
  exec_time = std::min(
      traj_duration,
  ...
  Eigen::Vector2d pos_err(
      pos_des(0) - odom_pos(0),
      pos_des(1) - odom_pos(1));
